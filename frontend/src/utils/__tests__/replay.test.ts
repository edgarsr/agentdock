...
describe('replay rendering rules', () => {
  ...
  it('preserves structured execute content while accumulating legacy output', () => {
    const data: ConversationReplayData = {
      sessions: [{
        sessionId: 'structured-output-session',
        adapterName: 'claude-code',
        prompts: [{
          events: [
            {
              ...
              toolRawJson: JSON.stringify({
                ...
                content: [
                  ...
                  { type: 'content', content: { type: 'text', text: 'compiling...' } },
                ],
              }),
            },
            {
    ...
    const raw = JSON.parse(toolCall?.entry.rawJson || '{}');

    expect(toolCall?.entry.result).toBe('compiling...\n\ndone');
    expect(raw.content.map((item: any) => item.type))
      .toEqual(['resource', 'content', 'content', 'image']);
  });

  it('preserves existing text and resources when an update only adds an image', () => {
    const data: ConversationReplayData = {
      sessions: [{
        sessionId: 'resource-only-update-session',
        adapterName: 'claude-code',
        prompts: [{
          events: [
            {
    ...
    expect(toolCall?.entry.result).toBe('done');
    expect(raw.content.map((item: any) => item.type)).toEqual(['content', 'resource', 'image']);
  });
